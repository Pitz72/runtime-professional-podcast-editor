...
export const useWaveformData = (
  ...
  return useMemo(() => {
    ...
    switch (quality) {
      case 'low':
        samplesPerPixel = Math.max(1, Math.floor(channelData.length / (width * 0.5))); // Half resolution
        break;
      case 'high':
        samplesPerPixel = Math.max(1, Math.floor(channelData.length / (width * 2))); // Double sampling
        break;
      case 'medium':
      default:
        samplesPerPixel = Math.max(1, Math.floor(channelData.length / width));
        break;
    }
  ...
  }, [audioBuffer, width, height, quality]);
};
